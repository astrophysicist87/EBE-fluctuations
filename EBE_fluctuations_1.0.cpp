...
#include "declarations_1D.h"
#include "generate_vector_1D_old.h"
//#include "filename_new.h"
//#include "outfilename_1.0.h"
#include "qng_1d_vec_old.h"
#include "gauss.h"
...
void fill_pos_coords(double a1, double b1, double a2, double b2, double a3, double b3, double a4, double b4)
{
...
void fill_fourier_transform_ens_avg(int dim)
{
	for (int i = 0;  i <= nq_points - 1; i++)
	{
		...
		for (int j = 0; j <= order-1; j++)
		{
			(*x_ptr).at(0) = (*grid_ptr).at(index(j,0,0,0)).x;
			(*temp_re_ptr).at(j) = fourier_kernel_real(q_ptr,x_ptr,1)*(*ens_avg_ptr).at(j);
			(*temp_im_ptr).at(j) = fourier_kernel_imag(q_ptr,x_ptr,1)*(*ens_avg_ptr).at(j);
		}
...
void fill_fourier_transform_model_function(int i, int dim, void * params_ptr)
{
		...
		{
			(*x_ptr).at(0) = (*grid_ptr).at(index(j,0,0,0)).x;
			(*temp_re_ptr).at(j) = fourier_kernel_real(q_ptr,x_ptr,1)*S_function((*x_ptr).at(0), params_ptr);
			(*temp_im_ptr).at(j) = fourier_kernel_imag(q_ptr,x_ptr,1)*S_function((*x_ptr).at(0), params_ptr);
		}
