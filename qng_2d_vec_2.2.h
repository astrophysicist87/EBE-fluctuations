#ifndef QNG_2D_VEC_2_2_H
#define QNG_2D_VEC_2_2_H
...
//if (my_flag) cout << i << "\t" << j << "\t" << (*local_wts_ptr2)[j] << endl;
		}
	}
	result_int *= half_length1 * half_length2;
...
double q_int2d_alt(vector<double>* vector_ptr)
{
	...
	for (int i = 0; i <= nq_points1-1; i++)
	{
		for (int j = 0; j <= nq_points2-1; j++)
		{
			...
			result_int_x += factor1*factor2*(*vector_ptr).at(nq_points2*i+j);
if (my_flag) cout << "INSIDE q_int2d_alt(): " << (*q_vec_ptr).at(nq_points2*i+j).qcoord1 << "\t" << (*q_vec_ptr).at(nq_points2*i+j).qcoord2 << "\t" << (*vector_ptr).at(nq_points2*i+j) << endl;
		}
result_int += result_int_x;
	}
	result_int *= q_interval1 * q_interval2;
//cout << "INSIDE q_int2d_alt(): " << result_int << "\t" << q_interval1 << "\t" << q_interval2 << endl;
